...
def residual_block(inputs,
    ...
    def residual_fn(inputs):
        if block_type == 'l2':
            residual = conv_fn(inputs, num_filters, 3, stride)
            residual = conv(residual, out_channels, 3, 1, activation_fn=None)
        elif block_type == 'l3':
            residual = conv_fn(inputs, num_filters, 1, stride)
    ...
    with tf.variable_scope(scope, 'residual_block_%s' % block_type, [inputs]):
        ...
        residual = residual_fn(inputs)
        return activation_fn(shortcut + residual)

residual_block_2 = functools.partial(residual_block, block_type='l2')
residual_block_3 = functools.partial(residual_block, block_type='l3')
